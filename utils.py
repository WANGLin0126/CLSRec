...
class BPRLoss:
    ...
    def stageOne(self, users, pos, neg, epoch):
        ...
        cl_loss =  self.cl_weight * self.model.cl_loss(users)  # contrastive loss
        ...
        self.opt.step()
        if epoch % 100 == 0:
            print(f"bpr_loss: {bpr_loss:.4e}, reg_loss: {reg_loss:.4e}, cl_loss: {cl_loss:.4e}")  #, cl_loss: {cl_loss:.4e}
...
def getFileName():
    ...
    elif world.model_name in ['LightGCN', 'CSRec']:
