...
# def get_user_item_matrix(SocialGraph, InteractionGraph):
#     # TODO: processing the  interaction graph select the users which have much social interactions but less item interatcions

#     num_users = SocialGraph.shape[0]
#     num_items = InteractionGraph.shape[0] - num_users

#     idx = SocialGraph.coalesce().indices()
#     val = torch.ones(idx.shape[1]).to(SocialGraph.device)
...
#     social_mask = social_count > 13  # 13 is the average iteractions of users

#     # due to the interaction graph is exclud the social edges, so the row counts of the interaction graph is the number of the item counts

#     items_count = []
#     for i in range(num_users):
#         items_count.append(InteractionGraph[i].coalesce().indices().shape[1])
#     avg_items = sum(items_count) / len(items_count)
#     item_mask = torch.tensor(items_count) < avg_items
#     item_mask = item_mask.to(SocialGraph.device)

#     # selecting user who have many social interactions but less item interactions
#     selected_users = torch.logical_and(social_mask, item_mask)
#     selected_users = selected_users.nonzero().squeeze()

#     new_social_graph = SocialGraph[selected_users[0]].to_dense()[selected_users].reshape(1,-1)
#     for i in selected_users[1:]:
#         i_row = SocialGraph[i].to_dense()
#         i_row = i_row[selected_users]
#         new_social_graph = torch.cat([new_social_graph, i_row.reshape(1,-1)],dim=0)
#     new_social_graph = (new_social_graph > 0).to(torch.float32)

#     # according to the selected users, extract a new user-item matrix\
#     new_user_item_matrix = InteractionGraph[selected_users[0]].to_dense().reshape(1,-1)
#     for i in selected_users[1:]:
#         i_row = InteractionGraph[i].to_dense()
#         new_user_item_matrix = torch.cat([new_user_item_matrix, i_row.reshape(1,-1)],dim=0)
#     new_user_item_matrix = (new_user_item_matrix > 0).to(torch.float32)
#     return new_user_item_matrix, selected_users
